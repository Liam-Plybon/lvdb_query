...
def query(keys,params):
    ...
    #define yes/no variables-- when dist, stru, or kine become active, i.e =1, this indicates
...
def between(param,minv,maxv):
    ...
    master_header=['dist_mod','dist_mod_em','dist_mod_ep','method','ref',
                   'ra','ra_em','ra_ep','dec','dec_em','dec_ep','ellipticity','ellipticity_em','ellipticity_ep',
                   'position_angle','position_angle_em','position_angle_ep','rscale','rscale_em','rscale_ep',
                   'rparam_2','rparam_2_em','rparam_2_ep','rhalf','rhalf_em','rhalf_ep','m_v','m_v_em','m_v_ep',
                   'apparent_magnitude','apparent_magnitude_em','apparent_magnitude_ep','model',
                   'helio_velocity','helio_velocity_em','helio_velocity_ep','velocity_dispersion',
                   'velocity_dispersion_ep','velocity_dispersion_em','n']#contains table names as well
    ...
    #if param in master_header == True:
    #    pass
    #else:
    #    raise IOError('Parameter does not exist: ' + str(param))
    ...
    if param in dist_header:
        dist=1
    if param in kine_header:
        kine=1
    if param in stru_header:
        stru=1

        
    #create queries as strings, then add to lists
    if dist == 1:
        key_out=db.select('SELECT key FROM distance WHERE ' + str(param) + ' BETWEEN ' + str(minv) + ' AND ' + str(maxv) + ';')
    if kine == 1:
        key_out=db.select('SELECT key FROM kinematics WHERE ' + str(param) + ' BETWEEN ' + str(minv) + ' AND ' + str(maxv) + ';')
    if stru == 1:
        key_out=db.select('SELECT key FROM structure WHERE ' + str(param) + ' BETWEEN ' + str(minv) + ' AND ' + str(maxv) + ';')
    
    new=[]
    for x in key_out:
        new.append(str(x).replace('(\'','').replace('\',)',''))
